<template>
    <div v-if="modelValue" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-sm w-full p-7 space-y-5 relative animate-fade-in">
            <!-- Close Button -->
            <div @click="close" class="absolute top-3 right-3 text-gray-400 hover:text-gray-900 transition-colors">
            ...
            <form v-if="!isSignUp" @submit.prevent="handleSignIn" class="space-y-3">
                ...
                <div>
                    ...
                    <input v-model="signInEmail" type="email" id="signin-email" placeholder="your@email.com"
                        class="w-full pl-2 py-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-gray-400 focus:border-transparent text-xs"
                        required />
                </div>
                ...
                <div>
                    ...
                    <input v-model="signInPassword" type="password" id="signin-password" placeholder="Password"
                        class="w-full pl-2 py-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-gray-400 focus:border-transparent text-xs"
                        required />
                </div>
                ...
                <!-- Sign In Button -->
                <button type="submit" :disabled="isLoading" :class="['w-full py-2.5 rounded-md font-medium text-xs transition-all',
                    isLoading
            ...
            <form v-else @submit.prevent="handleSignUp" class="space-y-3">
                ...
                <div>
                    ...
                    <input v-model="signUpName" type="text" id="signup-name" placeholder="John Doe"
                        class="w-full pl-2 py-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-gray-400 focus:border-transparent text-xs"
                        required />
                </div>
                ...
                <div>
                    ...
                    <input v-model="signUpEmail" type="email" id="signup-email" placeholder="your@email.com"
                        class="w-full pl-2 py-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-gray-400 focus:border-transparent text-xs"
                        required />
                </div>
                ...
                <div>
                    ...
                    <input v-model="signUpPassword" type="password" id="signup-password" placeholder="Password"
                        class="w-full pl-2 py-2 rounded-md border border-gray-300 focus:ring-2 focus:ring-gray-400 focus:border-transparent text-xs"
                        required />
