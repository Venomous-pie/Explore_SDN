      ...
      <div class="lg:col-span-2 space-y-3">
        ...
        <div class="relative w-full h-[320px] bg-gray-200 rounded-xl overflow-hidden group">
          <div v-if="currentPlace && currentImages.length > 0" class="relative w-full h-full">
            <!-- Main Image -->
            <img :src="getImageSrc(currentImages[currentImageIndex])" :alt="currentPlace.name"
              class="w-full h-full object-cover" />

            <!-- Image Navigation Arrows -->
            <div v-if="currentImages.length > 1" @click="previousImage"
              class="w-5 h-5 flex items-center justify-center absolute left-4 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white p-2 rounded-full shadow-lg transition-all opacity-0 group-hover:opacity-100"
            ...
            <div v-if="currentImages.length > 1" @click="nextImage"
              class="w-5 h-5 flex items-center justify-center absolute right-4 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white p-2 rounded-full shadow-lg transition-all opacity-0 group-hover:opacity-100"
            ...
            <div v-if="currentImages.length > 1" class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
              <button v-for="(_, index) in currentImages" :key="index" @click="currentImageIndex = index" :class="[
        ...
        <div v-if="currentPlace" class="space-y-2">
          ...
          <div class="flex gap-3 pt-2">
            <div @click="handleBookNow"
              class="flex-1 bg-black text-white font-semibold py-2.5 px-6 rounded-full text-center outline outline-1 outline-black transition-colors shadow-lg hover:shadow-xl text-sm">
              Book Now
            </div>
            <div @click="handleLearnMore"
              class="flex-1 bg-white hover:bg-gray-50 outline outline-1 outline-black text-gray-900 font-semibold py-2.5 px-6 rounded-full text-center transition-colors text-sm">
              Learn More
            </div>
          </div>
        </div>
      ...
      <div class="space-y-3">
        ...
        <div class="bg-white rounded-xl shadow-lg p-4">
          ...
          <div class="space-y-1.5 max-h-[180px] overflow-y-auto">
            <div v-for="municipality in municipalities" :key="municipality" @click="selectMunicipality(municipality)"
              :class="[
                'mx-3 my-2 text-left px-3 py-2 rounded-lg transition-colors font-medium text-sm',
        ...
        <div v-if="availablePlaces.length > 0" class="bg-white rounded-xl shadow-lg p-4">
          ...
          <div class="space-y-1.5 max-h-[280px] overflow-y-auto">
            <div v-for="(place, index) in availablePlaces" :key="place.id" @click="selectPlace(index)" :class="[
              'mx-3 text-left px-3 py-2 rounded-lg transition-colors',
